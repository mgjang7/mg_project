  ...
  <div class="w-full flex justify-center content overflow-scroll">
    <div class="container-box">
      <div class="w-full mb-16">
        <p class="main-title font-notosans">부대매출 현황</p>
      </div>
      ...
      <!-- 테이블 -->
      <div id=incidentalSalesStatus class="w-full h-full mb-5">
        <a-table
          :columns="columns"
    ...
    <!-- Modal -->
    <!-- <IncidentalSalesAddModal v-if="showModal" @modalClose="showModal = false"> -->
...
export default class IncidentalSalesStatus extends Vue {
  ...
  goDetail(month:number) {
    // this.$router.push({path: '/incidentalRevenueManagement/incidentalSalesStatus/detail', query: {getMonth: month.toString()}});
  ...
  async getIncidentalSalesStatus() {
    try {
      const respones = await this.$axios.get(this.SalesUrl + '/incidentalSalesStatus', {
        params: {
      ...
      if(respones.data.data !== null && respones.data.data !== undefined && respones !== null && respones.data !== null) {
        ...
        alert('부대매출 현황 데이터 조회 실패');
      }
    } catch(e) {
      console.log('부대매출 현황 데이터 조회 실패 : ' + e);
      alert('부대매출 현황 데이터 조회 실패');
    }
  }
  ...
  async search() {
    ...
    // 부대매출 현황 데이터 조회
    await this.getIncidentalSalesStatus();
  }
...
#incidentalSalesStatus .ant-table-pagination.ant-pagination {
  display: none;
}
</style>